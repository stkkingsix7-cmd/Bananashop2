...
if (!process.env.TOKEN) {
  console.error("❌ ERROR: No se ha definido la variable de entorno TOKEN");
  process.exit(1); // Detiene el bot si no hay token
}
...
client.on("messageCreate", async message => {
  ...
  if (command === "rename") {
    ...
    if (now - last < 1500) return message.reply("⏳ Espera 2s antes de renombrar otra vez.");
  ...
  if (command === "addemoji") {
    ...
    if (!input) return message.reply("❌ Usa: ,addemoji <emoji|id|url> [nombre]");
    ...
    if (!url && /^\d{10,20}$/.test(input)) url = `https://cdn.discordapp.com/emojis/${input}.png`;
    if (!url && input.startsWith("http")) url = input;
    if (!url) return message.reply("❌ Emoji inválido.");

    try {
      const res = await fetch(url);
      if (!res.ok) throw new Error("HTTP " + res.status);
      let buffer = Buffer.from(await res.arrayBuffer());
      if (url.endsWith(".webp")) buffer = await sharp(buffer).png().toBuffer();
...
client.on("interactionCreate", async interaction => {
  if (interaction.isButton()) {
    if (interaction.customId === "copy_paypal") return interaction.reply({ content: PAYPAL_EMAIL, ephemeral: true });
    if (interaction.customId === "copy_ltc") return interaction.reply({ content: LTC_ADDRESS, ephemeral: true });
    if (interaction.customId === "copy_btc") return interaction.reply({ content: BTC_ADDRESS, ephemeral: true });
  }
